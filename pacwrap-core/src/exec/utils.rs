...
use crate::{constants::{LOG_LOCATION, BWRAP_EXECUTABLE, PACWRAP_AGENT_FILE, GID, UID, TERM, LANG, COLORTERM}, 
    config::InstanceHandle, 
    ErrorKind};

lazy_static! {
    static ref ID: (&'static str, &'static str) = (UID.to_string().leak(), GID.to_string().leak());
}
