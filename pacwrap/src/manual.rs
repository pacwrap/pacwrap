...
fn pacwrap_yml(buf: &mut String, layout: &HelpLayout) -> Result<(), std::fmt::Error> {
    ...
    match layout {
        HelpLayout::Man =>
            writeln!(buf, ".nh\n.TH {name}.yml 2 \"{date}\" \"{name} {version}\" \"Pacwrap Configuration Directives\"\n")?,
...
fn default(buf: &mut String, layout: &HelpLayout) -> Result<(), std::fmt::Error> {
    ...
    match layout {
        HelpLayout::Man => writeln!(buf, ".nh\n.TH {name} 1 \"{date}\" \"{name} {version}\" \"User Manual\"\n")?,
        HelpLayout::Markdown => writeln!(
            buf,
            "# Pacwrap User Manual
...
{tab}This application is designed to allow for the creation and execution of secure, replicable 
...
{sub_text}This documentation was generated by {name} v{}.
