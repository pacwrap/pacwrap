...
{tab}Synchronize package databases and update packages in target containers. 

{sub}-U, --utils{reset_bold}
{tab}Invoke miscellaneous utilities to manage containers.

{sub}-P, --process{reset_bold}
{tab}Manage and show status of running container processes.

{sub}-E, --execute{reset_bold}
{tab}Executes application in target container using bubblewrap.

{sub}-h, --help{reset_bold}
{tab}Invoke a printout of this manual to {bold}stdout{reset_bold}. Specify an option verbatim for further information.

{sub}-V, --version{reset_bold}
{tab}Display version and copyright information in {bold}STDOUT{reset_bold}.\n");
}
...
fn execute(layout: &HelpLayout) {
    ...
    println!("{head}EXECUTE{reset}

{sub}-r, --root{reset_bold}
{tab}Execute operation with fakeroot and fakechroot. Facilitates a command with faked privileges.
	
{sub}-s, --shell{reset_bold}
{tab}Invoke a bash shell\n");
}
...
{tab}When specifying a topic to display, show the default topic in addition to specified options.

{sub}-f, --format=FORMAT{reset_bold}
{tab}Change output format of help in {bold}STDOUT{reset_bold}. Format options include: 'ansi', 'dumb', 'markdown', and 'man'. 
{tab}This option is for the express purposes of generating documentation at build time, and has little utility
{tab}outside the context of package maintenance. 'man' option produces troff-formatted documents for man pages.
{tab}

{sub}-a, --all, --help=all{reset_bold}
{tab}Display all help topics.\n");
}
...
fn sync(layout: &HelpLayout) {
    ...
    println!("{head}SYNCHRONIZATION{reset}

{sub}-y, --refresh{reset_bold}
{tab}Synchronize remote package databases. Specify up to 2 times to force a refresh.

{sub}-u, --upgrade{reset_bold}
{tab}Execute aggregate upgrade routine on all or specified containers. Use {bold}-t, --target=TARGET{reset_bold} to limit
{tab}package synhronization operations to the specified target containers. Packages applicable to a target {bold}must{reset_bold}
{tab}be specified only after the target operand. e.g. '-t electron element-desktop -t mozilla firefox thunderbird'

{sub}-f, --filesystem{reset_bold}
{tab}Force execution of filesystem synchronization target on all or specified containers. In combination
{tab}with {bold}-o/--target-only{reset_bold}, in addition to no other specified targets, filesystem slices will be
{tab}synchronized without package synchronization on all applicable containers.

{sub}-c, --create{reset_bold}
{tab}Create a container with the first specified target. A container type argument is also required.

{sub}-b, --base{reset_bold}
{tab}Base container type. Specify alongside {bold}-c, --create{reset_bold} to assign this container type during creation.
{tab}This container type is used as the base layer for all downstream containers. Only one base container 
{tab}dependency per slice or per root is supported. Filesystem and package deduplication via slices and root 
{tab}containers is recommended, but optional.

{sub}-d, --slice{reset_bold}
{tab}Slice container type. Specify alongside {bold}-c, --create{reset_bold} to assign this container type during creation.
{tab}Requires a base dependency target, and optionally sliced dependency target(s), in order to ascertain
{tab}foreign packages and influence ordering of downstream synchronization target(s). Container slicing provides 
{tab}the ability to install packages in a lightweight, sliced filesytem, which aid in the deduplication of 
{tab}common downstream package and filesystem dependencies (e.g. graphics drivers, graphical toolkits, fonts, 
{tab}and display server dependencies, etc.).

{sub}-r, --root{reset_bold}
{tab}Root container type. Specify alongside {bold}-c, --create{reset_bold} to this assign container type during creation.
{tab}Requires a base dependency target, and optionally sliced dependency targets, in order to ascertain
{tab}foreign packages and influence ordering of this target. These containers are ideal for installing
{tab}software in with the least amount of filesystem and package synchronization overhead.

{sub}-t, --target=TARGET{reset_bold}
{tab}Specify a target container for the specified operation.

{sub}-o, --target-only{reset_bold}
{tab}Apply specified operation on the specified target only.

{sub}--force-foreign{reset_bold}
{tab}Force synchronization of foreign packages on resident container. Useful for when installing 
{tab}a new package in a root container without all the prerequisite foreign dependencies synchronized 
{tab}to this container's package database.

{sub}--dbonly{reset_bold}
{tab}Transact on resident containers with a database-only transaction.

{sub}--noconfirm{reset_bold}
{tab}Override confirmation prompts and confirm all operations.\n");
...
fn version(layout: &HelpLayout) {
    ...
    println!("{head}VERSION{reset}

{sub}-v, --version, --version=min{reset_bold}
{tab}Sends version information to {bold}STDOUT{reset_bold} with colourful ASCII art. 
{tab}The 'min' option provides a minimalistic output as is provided to non-colour terms.

{tab}This documentation was generated by {name} v{version_num}-{suffix}-{release} ({timestamp}).
...
{tab}Copyright (C) 2023 - Xavier R.M.
